...
Why using type in interpreting program? Well, it's like an efficiency boost
classifier in deconstructing/running code. Most of the functions we've seen can
be roughly allocated to several categories. If we can write an abstract and
universal function for each type, we can call them and optimal each function
based on its characteristics. 
 
Type is allocated based on the work essence of characters in the context. There are six types:
=(const, quote, identifier, lambda, cond, application)=. The expression is put
in the function =(expression-to-action)= to allocate in to finer categories so
that we can implement it to 
[[/img/little11.png]]
The above tree code blocks can be used with the below two function as link:
#+begin_src scheme
