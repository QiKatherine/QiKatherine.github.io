...
For example, =(rember a l)= removes multiple occurrences =a= as the /first level/ s-expression of list =l=, whereas =(rember* a l)= 
...
The below functions do a bit more, they return intersection and union results.
#+begin_src scheme
