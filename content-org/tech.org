...
parameter has name of /lset/, which is the same with the inner parameter name
/lset/. This is a coincidence, and the name in A can use something else a
name holder. The value of this name holder will be substituted by the /lset/ of =(else (A lset))=.
#+begin_src scheme
