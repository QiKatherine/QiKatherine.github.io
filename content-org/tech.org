...
and /lat/, you would naturally ask, what if I use currying learned from The
...
finding =(mr)= function, it will substitute /a/ with /'c/ and /lat/ with /'(c d)/, which will report error
...
defining a local recursive function that knows the definition of /a/, then we implement the function
to list /lat/.
