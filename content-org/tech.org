...
But you might have figured already: the partial function detect function
=(will-stop?)= do NOT exist. The scientists made a very delicate example to show
this. 
...
argument if we change the *define* to *lambda*. The difference is, the =(length)= function will return a
value whereas the =(λ length)= will return a function. It is the returned
function =(λ l)= that returns value.
#+begin_src scheme
(lambda (length)
 (lambda (l)
  (cond
...
As you read more, you would realize
the length measure function is a carefully selected example: it does a simple job at a time; applying
it on itself is equal to doing a simple job on a achieved job; when repeating calling itself,
it can achieve a almost infinite loop.

In addition, we change =(length)= a little by calling =(eternity)=
instead of =(length)= in the most inner recursion. This makes the function only works on *null input*, since it will terminate in the =(null?)= predicate without calling the partial
=(eternity)=, otherwise any non-null input will trigger the infinite
recursion and cause failure by giving no answer. This is essentially how the
=(length<=0)= can only measure length of list with *zero* element. If we want to
...
** TODO The Seasoned Schemer                                          :@TECH:
   :PROPERTIES:
