...
For example, the =(pick)= will return the n-th
element in lat. The =(keep-looking)= calls =(pick)= to see if =a= is equal to
the random atom in lat (assuming the numbers in lat is random).
#+begin_src scheme
...
is contained in the list, so the recursion will run without stop, which is
called *partial function* as opposed to *total function* defined by us previously. 

Let's see another example. We've defined *pair* is a list containing two
s-expressions. The =shift= takes a pair whose first component is a pair and builds a pair by shifting the second part of the first component into
...
#+end_src
Based on =shift= we further creat =align=. Don't rush to run the function. We notice something wrong in the
 second predicate where =shift= is called as argument. The seventh commandment emphasize
 that recursion should happen on the subparts that are *of the same nature*:
 ...
 in recursion that is *not part* of the original argument, which will not make
 argument recursion get smaller or closer to any finishing line.

So we define two different ways to measure the mass of the first component. The
=(length*)= measures every atom with same weight, whereas the =(weight*)= puts
twice as much weight to the first component. 
#+begin_src scheme
(define length*
  (lambda (pora)
    (cond
