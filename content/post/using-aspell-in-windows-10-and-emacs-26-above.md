...
更新 Emacs 以后发现 aspell 不能用了，调用 `ispell-minor-mode` 时一直收到以上报错，查看以上答案发现 Emacs 26 以上的版本刚发布的时候没有 windows 匹配的 aspell 安装版本，所以当时解决方式时暂时用 hunspell 代替。现在有了解决方案： `用msys2安装aspell` 。
